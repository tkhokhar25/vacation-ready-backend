...
def filter_restaurants(restaurant_data):
    id_to_rating = {}
...
def get_restaurants(pricing, interest_set_id):
    ...
    for i in range (0, len(cuisines[0][0])):
        request_url = format_restaurant_request_url(cuisines[0][0][i] + " restaurants", "Chicago", price_level - 1, price_level)
        restaurant_data = requests.get(request_url).json()
        suggested_restaurants.append(filter_restaurants(restaurant_data))
